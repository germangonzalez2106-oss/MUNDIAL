...
# ==================== FUNCIONES ====================
def cargar_excel_a_mongodb():
    if coleccion_selecciones.count_documents({}) > 0:
        print(f"📊 MongoDB Atlas ya tiene {coleccion_selecciones.count_documents({})} selecciones")
        return
    
    print("🔄 Cargando datos del Excel a MongoDB Atlas...")
    ...
    for sheet in excel.sheet_names:
        if sheet in ['RESUMEN_GENERAL', 'Resumen']:
            continue
        ...
        seleccion_doc = {
            ...
            'asistencias_total': int(df['Asistencias'].sum()),
            'rating_promedio': round(df['Rating'].mean(), 2),
            'plantilla': plantilla,
            'ultima_actualizacion': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        }
        
        coleccion_selecciones.insert_one(seleccion_doc)
        print(f"   ✅ Insertada: {sheet}")
    
    print(f"✅ Carga completa. {coleccion_selecciones.count_documents({})} selecciones en MongoDB Atlas")
...
# ==================== INICIALIZACIÓN ====================
try:
    cargar_excel_a_mongodb()
except Exception as e:
    print(f"⚠️ No se pudo cargar datos a MongoDB Atlas: {e}")
...
def index():
    ...
    <head>
        ...
        <style>
            ...
            .resultados-busqueda h3, .comparacion-container h3 {
                margin-top: 0;
                color: #4CAF50;
            }
            .comparacion-grid {
        ...
        <script>
                ...
                fetch('/api/buscar?q=' + encodeURIComponent(termino))
                    ...
                    .then(data => {
                        ...
                        if (data.length === 0) {
                            ...
                            html += '<thead><tr><th>Jugador</th><th>Selección</th><th>Goles</th><th>Asistencias</th><th>Rating</th></tr></thead><tbody>';
                            for (let j of data) {
                                html += '<tr>';
            ...
            function compararSelecciones() {
                ...
                if (!eq1 || !eq2) {
                    alert("Selecciona dos equipos para comparar");
                    return;
                }
                ...
                fetch('/api/comparar?eq1=' + encodeURIComponent(eq1) + '&eq2=' + encodeURIComponent(eq2))
                    ...
                    .then(data => {
                        ...
                        html += '<h4>' + data.equipo1.nombre + '</h4>';
                        ...
                        html += '<div class="stat-row"><span>⚽ Máximo Goleador</span><span>' + data.equipo1.max_goleador.nombre + ' (' + data.equipo1.max_goleador.goles + ' goles)</span></div>';
                        html += '</div>';
                        
                        html += '<div class="vs">VS</div>';
                        
                        html += '<div class="comparacion-equipo">';
                        html += '<h4>' + data.equipo2.nombre + '</h4>';
                        ...
                        html += '<div class="stat-row"><span>⚽ Máximo Goleador</span><span>' + data.equipo2.max_goleador.nombre + ' (' + data.equipo2.max_goleador.goles + ' goles)</span></div>';
                        html += '</div>';
                ...
                fetch('/api/selecciones')
                    ...
                    .then(data => {
                        ...
                        rankingChart = new Chart(ctx, {
                            type: 'bar',
                            data: {
                                labels: top10.map(t => t.nombre),
                                datasets: [{
                                    label: 'Rating Promedio',
                                    data: top10.map(t => t.rating_promedio),
                            ...
                            options: {
                                ...
                                plugins: {
                                    ...
                                    title: { display: true, text: 'Top 10 - Rating Promedio', color: 'white' }
                                },
                                scales: {
                                    y: { title: { display: true, text: 'Rating', color: 'white' }, ticks: { color: 'white' } },
                                    x: { ticks: { color: 'white', rotation: 45, font: { size: 10 } } }
                                }
                            }
                        });
                    });
                    ...
                    .then(data => {
                        ...
                        new Chart(ctx, {
                            ...
                            data: {
                                ...
                                datasets: [{
                                    label: 'Goles Totales',
                                    data: top10.map(t => t.goles_total),
                            ...
                            options: {
                                ...
                                plugins: {
                                    legend: { labels: { color: 'white' } },
                                    title: { display: true, text: 'Top 10 - Goles Totales', color: 'white' }
                                },
                                scales: {
                                    y: { title: { display: true, text: 'Goles', color: 'white' }, ticks: { color: 'white' } },
                                    x: { ticks: { color: 'white', rotation: 45, font: { size: 10 } } }
                                }
                            }
                        });
    ...
    <body>
        ...
        <p>Datos almacenados en <strong>MongoDB Atlas ☁️</strong> | Total: 25 selecciones</p>
    ...
    html += """
        ...
        <div id="comparacion-container" class="comparacion-container">
    ...
    for i, seleccion in enumerate(selecciones, 1):
        html += f"""
            <tr>
                <td>{i}</td>
                <td><strong><a href="/seleccion/{seleccion['nombre']}">{seleccion['nombre']}</a></strong></td>
                <td>{seleccion['jugadores']}</td>
                <td>{seleccion['goles_total']}</td>
                <td>{seleccion['rating_promedio']}</td>
            </tr>
        """
...
def ver_seleccion(nombre):
    ...
    <html>
    <head>
        <title>{nombre} - Mundial 2026</title>
        <style>
            body {{ 
                font-family: Arial, sans-serif; 
                background: #1a1a2e; 
    ...
    <body>
        ...
        <div class="stats">
            <div class="stat-card">
                <h3>{seleccion['jugadores']}</h3>
                <p>Jugadores</p>
            </div>
            <div class="stat-card">
                <h3>{seleccion['goles_total']}</h3>
                <p>Goles Totales</p>
            </div>
            <div class="stat-card">
                <h3>{seleccion['asistencias_total']}</h3>
                <p>Asistencias</p>
            </div>
            <div class="stat-card">
                <h3>{seleccion['rating_promedio']}</h3>
                <p>Rating Promedio</p>
            </div>
        </div>
        ...
        <table>
            <thead>
                <tr><th>Jugador</th><th>Goles</th><th>Asistencias</th><th>Rating</th></tr>
            </thead>
            <tbody>
    """
    ...
    html += f"""
        ...
        <div class="update-info">
            📅 Última actualización: {seleccion.get('ultima_actualizacion', 'Desconocida')}
        </div>
...
def api_buscar():
    ...
    if len(termino) < 2:
        return jsonify([])
    resultados = buscar_jugadores(termino)
...
if __name__ == '__main__':
    ...
    print("🔍 Buscador de jugadores integrado")
    print("⚔️ Comparador de selecciones integrado")
    print("📊 Gráficos de ranking y goles")
